...
function DisplayShopDetails({ details }) {
  ...
  return (
    <div className="w-full px-4 py-4">
      <p className="ml-2 mb-5 text-2xl font-bold">{name}</p> {/* 폰트 크기와 마진 조정 */}
      <div className="flex items-center mb-3"> {/* 아이템들을 중앙에 정렬하고 마진 조정 */}
        <img src={floorIcon} className="h-4 mr-5 ml-2" alt="Floor icon" /> {/* 아이콘 크기 및 마진 조정 */}
        <p className="text-base">{floor}층</p> {/* 텍스트 크기 조정 */}
      </div>
      <div className="flex items-center mb-3">
        <img src={numberIcon} className="h-4 mr-5 ml-2" alt="Phone icon" /> {/* 아이콘 크기 및 마진 조정 */}
        <p className="text-base">{phone}</p>
      </div>
      <div className="flex items-center mb-6 text-base">
        <img src={ratingIcon} className="h-4 mr-4 ml-2" alt="Rating icon" /> 
        <InputRatingStar rating={avgScore} readOnly={true} isStoreStar={true} starSize={22} /> 
