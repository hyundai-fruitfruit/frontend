...
import HeadupNotification from 'components/Notification/HeadupNotification';
...
function MainPage() {
  ...
  const { isOutOfArea } = useCurrentLocation();
  ...
  const handleMenuIconClick = (iconName) => {
    if (iconName === 'start') {
      // '모험 시작' 아이콘 클릭 시 위치 확인 로직
      if (!isOutOfArea) {
        // 사용자가 목적지 내에 있다면 Notification 표시
        setShowNotification(true);
        setTimeout(() => setShowNotification(false), 3000); // 3초 후 Notification 숨김
      }
    } else {
      // 다른 아이콘 클릭 시 모달창 표시 로직
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4 ">
      ...
      {showNotification && <HeadupNotification message="위치 인증이 성공하였습니다!" type="success" />}{' '}
      <div className="flex flex-nowrap justify-center items-center relative">
        <IconMenu
          icons={iconsData}
          onIconClick={handleMenuIconClick} // 수정된 부분
        />
      </div>
    </div>
  );
